...
void scheduler() {
    ...
    while (!taskBuffer.empty()) {
        ...
        for (int i = 0; i < totalConsumers; ++i) {
            ...
            std::cout << "Scheduler distributing " << sharedtask << " tasks to Consumer " << cons.getId() << std::endl;
...
void consumer (int id){
    ...
    while (!schedulersFinished) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100)); // Sleep to prevent race conditions
    }
